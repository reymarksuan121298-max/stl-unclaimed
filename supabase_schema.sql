...
CREATE TABLE IF NOT EXISTS "OverAllCollections" (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    unclaimed_id BIGINT REFERENCES "Unclaimed"(id) ON DELETE SET NULL,
    teller_name TEXT NOT NULL,
    bet_number TEXT NOT NULL,
    draw_date DATE NOT NULL,
...
BEGIN
    IF (OLD.status = 'Unclaimed' AND NEW.status = 'Collected') THEN
        ...
        INSERT INTO "OverAllCollections" (
            ...
            'System', -- Default collector
            NEW.franchise_name
        );

        -- Insert into Reports
        INSERT INTO "Reports" (
            teller_name, amount, collector, area,
            staff_amount, collector_amount, agent_amount, admin_amount
        ) VALUES (
            NEW.teller_name, NEW.win_amount, 'System', NEW.area,
